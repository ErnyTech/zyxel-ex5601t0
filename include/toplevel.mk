...
LAST_PROJECT := $(strip $(shell cat $(TOPDIR)/.last_project 2>/dev/null))
...
IGNORE_CHECK_ZYPREPARE=1
endif

zyprepare: scripts/config/conf
	$(info $@: P=$(P))
	@if [ "$(P)" = "" ]; then \
          echo "You need to specify build project name from defconfig using 'make P=<project name>...'"; exit 1; \
	fi
	@if [ "$(LAST_PROJECT)" = "" ]; then \
		$(TOPDIR)/scripts/private/zyxel/preCfg.sh $(P); \
		echo "$(P)" > $(TOPDIR)/.last_project; \
		export KCONFIG_OVERWRITECONFIG=1; scripts/config/conf --defconfig=.config Config.in; \
	else \
		if [ "$(LAST_PROJECT)" != "$(P)" ]; then \
			echo "The specified project, $(P), differs from the last one built, $(LAST_PROJECT)."; \
...
.config: ./scripts/config/conf $(if $(CONFIG_HAVE_DOT_CONFIG),,prepare-tmpinfo) $(if $(IGNORE_CHECK_ZYPREPARE),,zyprepare)
...
fcs:
ifeq ($(R),)
	@$(_SINGLE)$(NO_TRACE_MAKE) -C target/linux fcs
	...
	@$(_SINGLE)$(NO_TRACE_MAKE) -C $(TOPDIR) package/private/zyxel/zcfg/{clean,compile} R=$R
	@$(_SINGLE)$(NO_TRACE_MAKE) -C target/linux fcs
	@$(_SINGLE)$(SUBMAKE) -r package/index
	@$(_SINGLE)$(SUBMAKE) -r json_overview_image_info
	@$(_SINGLE)$(SUBMAKE) -r checksum
...
distclean:
	rm -rf bin build_dir .ccache .config* dl key-build* logs package/openwrt-packages staging_dir tmp
	rm -rf .last_project
