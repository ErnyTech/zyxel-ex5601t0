...
if [ ! -n "$board" -o ! -r "$kernel" -a  ! -r "$rootfs" -a  ! -r "$zyfwinfo" -a  ! -r "$zydefault" -o ! "$outfile" ]; then
	echo "syntax: $0 [--board boardname] [--kernel kernelimage] [--rootfs rootfs] [--zyfwinfo zyfwinfo] [--zydefault zydefault] out"
	exit 1
fi
