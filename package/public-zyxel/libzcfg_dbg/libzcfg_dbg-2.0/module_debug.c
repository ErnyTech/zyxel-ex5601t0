...
int debug_becellintf = 0;
int debug_libzcfg_be_cellintf = 0;
int debug_beconnectivity = 0;
int debug_berebootSchedule = 0;
...
int debug_bexmpp = 0;
int debug_begwinfo = 0;
int debug_befeatureFlag = 0;
...
{"bevoiceFxsCid", &debug_bevoiceFxsCid},
...
{"bexmpp", &debug_bexmpp},
{"begwinfo", &debug_begwinfo},
{"befeatureFlag", &debug_befeatureFlag},
{"libzcfg_be_cellintf", &debug_libzcfg_be_cellintf},
{"berebootSchedule", &debug_berebootSchedule},
...
void zcfg_save_tmp_module_debug_level()
{
	zcfg_dbg_module_list_t *dbg_p = &zcfg_dbg_module_list[0];
	FILE *fdbg=fopen("/tmp/.debugModule.csv","w");
	
	if(fdbg==NULL){
		printf("open file fail\n");
	}
	else{		
		while( dbg_p-> module != NULL && dbg_p-> level != NULL){
			fprintf(fdbg,"%s,%d\n",dbg_p->module,*dbg_p->level);
			dbg_p++;
		}
		fclose(fdbg);
	}
...
void zcfg_restore_module_debug_level()
{
	char module[128];
	int level=0;
	FILE *fdbg = NULL;
	
	fdbg=fopen("/data/.debugModule.csv","r");
	if( fdbg != NULL){
		while(!feof(fdbg)){
			fscanf(fdbg,"%[^,], %d\n", module,&level);
			if(level){
				zcfg_set_module_debug_level( module, level);
			}
		}
		fclose(fdbg);
	}
