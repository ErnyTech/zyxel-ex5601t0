...
)
{
	...
	if(ptr != NULL){
		recordZcfgAllocData((void*)ptr, fileName, line, 0);
	}
#endif
...
)
{
    ...
    *recvBuf = (zcfgMsg_t *)_msg_alloc(recv_len + 1, __FILE__, __LINE__);
    if (*recvBuf == NULL)
    {
...
#ifdef ZCFG_MEMORY_LEAK_DETECT
	if(*recvBuf != NULL){
		recordZcfgAllocData((void*)*recvBuf, fileName, line, 0);
	}
#endif
    /*
...
)
{
    ...
    return _msg_recv(my_entry->eid, my_entry->sck_id, true, recvBuf, timeout_msec, 0, 0, 0, fileName, line);
}
...
)
{
    ...
    */
    ...
    // receive message
    ret = _msg_recv(eid, fd, false, replyMsg, timeout_msec, dst_eid, send_type, send_oid, __FILE__, __LINE__);
...
#ifdef ZCFG_MEMORY_LEAK_DETECT
	if(*replyMsg != NULL){
		recordZcfgAllocData((void*)*replyMsg, fileName, line, 0);
	}
#endif
