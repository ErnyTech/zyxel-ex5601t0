...
#include <sys/time.h>
#include <pthread.h> //For the pthread-based MUTEX related data type & macro: pthread_mutex_t, PTHREAD_MUTEX_INITIALIZER.
...
typedef struct CMTimer_t {
	...
	timeval_t	timeout;		/* timeout time of timer */
...
enum { 
	...
	rebootTimerID,
	logFileSizeCheckTimerID,
	zyMultiWanInitTimerID,
	zyMultiWanDelayDownTimerID,
	zyMultiWanAutoHuntTimerID,
	wifiChannelHoppingTimerIDBase,
	wifiChannelHoppingTimerIDEnd = wifiChannelHoppingTimerIDBase + 8 // reserve 8 ID for wifi channel hopping
};
