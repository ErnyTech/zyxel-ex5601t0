...
#define CCM_LOGS_TYPE_OUTCALL_BLOCK				9
#define CCM_LOGS_TYPE_INCALL_BLOCK				10
...
//--------------------------------------------------------------------------------------------------------------------
#define CALL_FEATURE_CALL_BLOCK				39 	/* Check Incoming or Dial out number is match with Call Block Rule */
//--------------------------------------------------------------------------------------------------------------------
#define CALL_FEATURE_NUMBERS		40
//==========================================================================================
...
#define CCM_CONN_TYPE_BLOCKED_CALL	10
#define CCM_CONN_TYPE_MAX				11 //Michael.20140223.001: Add for the debug message for the CCM Associator Event handling.
...
#define MAX_MATCHED_FEATURE_KEY_LENGTH	(42+1)	/*The same as prefixRange of VoiceProfNumPlanPrefixInfoObj_t*/
...
//#endif
	...
	//#ifdef ZYXEL_CALL_POLICY_HANDLE_SUPPORT
		EVT_CCM_ASSIGN_FAIL_CALL_BLOCKED		= EVENT( EVT_VOIP_CCM, 0x63),
	//#endif
	//------------------------------------------------------------------------------------------
	EVT_CCM_MAX_ID
} ccmEvent_et;
...
typedef struct dialDigitNum_s
{
	...
	char matched_feature_key[MAX_MATCHED_FEATURE_KEY_LENGTH]; //20210304.Anna.Add to store the matched feature key.
	uint8 matched_feature_key_length;
} dialDigitNum_t;
...
#if 1
bool checkEmergencyNumberPhBook(int VpItr, dialDigitNum_t *dialDigitNum_p);
#else
...
#ifdef ZYXEL_CALL_POLICY_HANDLE_SUPPORT
void checkCallBlockFeature(ccmAssociator_t *ccmAsso, int *retCode, int *exitFlag);
void ccmCallBlockRoute(ccmAssociator_t *ccmAsso, int *retCode, int phoneFeature, int *exitFlag);
...
//Amber.20210122.B: Add the common phone number related define and functions for parsing and checking emergency number and call block's white list.
#define COMMON_PHONE_NUMBER_SEPARATORS "|, \t"
typedef struct commonPhoneNumber_s
{
	char *str;
	int str_len;
	int *digitBitmapStr;
	struct commonPhoneNumber_t *next;
}commonPhoneNumber_t;

typedef struct commonPhoneNumberList_s
{
	uint32 list_count;
	uint32 max_phoneNumberLength;
	uint32 min_phoneNumberLength;
	commonPhoneNumber_t *phoneNumberList;
}commonPhoneNumberList_t;

int commonPhoneNumberListParser(char* inputPhoneNumberStr, char* delimiter, commonPhoneNumberList_t* outputPhoneNumberLinkList, bool ifTransferDigitBitmapStr);
bool commonPhoneNumberListCheckExactStrMatch(commonPhoneNumberList_t *checkedNumberList, char *targetStr, int targetStrLen);
int commonPhoneNumberListClean(commonPhoneNumberList_t* phoneNumberLinkList);
int commonPhoneNumberClear(commonPhoneNumber_t **start);
void commonPhoneNumberListShow(commonPhoneNumberList_t* phoneNumberLinkList);
int commonPhoneNumberInsertToDialPlan(char* dialPlanPtr, int dialPlanTotalSize, commonPhoneNumber_t* numberListHead, bool needImmediateDial);
//Amber.20210122.E: Add the common phone number related define and functions for parsing and checking emergency number and call block's white list.


#ifdef ZYXEL_CALL_POLICY_HANDLE_SUPPORT
#define COUNTRY_CALLING_CODE_REGEX "^(1|(12((4[26])|(6[48])|(84)))|(134[05])|(14((41)|(73)))|(16((49)|(58)|(64)|(7[01])|(84)))|(17((21)|(58)|(67)|(8[47])))|18(([024]9)|(6[89]))|1939|(2[07])|(21[1-368])|(22[0-9])|(23[0-9])|(24[0-9])|(25[0-8])|(26[0-9])|(29[017-9])|(3[0-46])|(35[0-9])|(37[0-9])|(38[0-35-9])|(4[013-9])|(42[013])|(5[1-8])|(50[0-9])|(59[0-9])|(6[0-6])|(67[02-9])|(68[0-35-9])|(69[0-2])|7|(7[346-9])|(8[1246])|(80[08])|(85[02356])|(87[08])|(88[0-368])|(9[0-58])|(96[0-8])|(97[0-79])|(99[1-8]))$"
#define COUNTRY_CALLING_CODE_LENTH_MAX 4 // TODO: Need to SYNC with the other related definition: 'region_zyxel.h/ZYXEL_VOIP_INTL_TELEPHONE_NUM_CALLING_CODE_LEN (16)'??

#define PHONE_NUMBER_CALL_TYPE_INTERNATIONAL_CALL 1
#define PHONE_NUMBER_CALL_TYPE_NATIONAL_CALL 2
#define PHONE_NUMBER_CALL_TYPE_REGULAR_CALL 3

#define NON_PHONE_NUMBER_SYMBOLS " ()-\t"

int getIntlTelephonNumCallingCodeBy(int VpItr /*IN*/, char** countryRegionIntlCallingCode_ptr /*OUT*/);
bool carrierSelectCodeListLoad(int VpItr);
bool checkCountryCallingCode(char *parseString, int *codeLenth);
bool checkPrefixBitMap(char *inputStr, int sipProfId, int *carrierSelecCodeLength);
bool checkPrefix(char *str, char *pre);
void trimSymbols(char *stringInput);
int transferPhoneNumberToUnifiedFormat(char *targetNumber_raw, char *outputNumber, int length);
bool checkCallBlockRule(char* targetNumber, int direction);
bool callPolicyExceptNumberListLoad(void);
bool callPolicyExceptNumberListCheck(char *targetStr, uint8* targetStrLen);
#endif //ZYXEL_CALL_POLICY_HANDLE_SUPPORT.


#if ZYXEL_VOIP_EMERGENCY_NUMBER_PHONEBOOK_SUPPORT
bool emergencyNumberListLoad(int VpItr);
bool emergencyNumberListCheck(int VpItr, char* targetStr, uint8* targetStrLen);
#endif //ZYXEL_VOIP_EMERGENCY_NUMBER_PHONEBOOK_SUPPORT.
