...
#define MAP_CONTROLLER_MSG_PORT   49998
#define ZY_SL_CLI_PORT            49997

// Sned event to driver mesh deamon to reduce trigger command
#define ZY_SL_EVT_PORT            49996
#define MAP_CONTROLLER_EVT_PORT   49995

struct zySL_mesh_interface_ctrl
{
  int s;
...
#if (MAX_RADIO_PER_DEVICE == 0)
#undef MAX_RADIO_PER_DEVICE
#define MAX_RADIO_PER_DEVICE    2
#endif
#if (MAX_BSS_PER_DEVICE == 0)
#undef MAX_BSS_PER_DEVICE
#define MAX_BSS_PER_DEVICE      10
#endif
#if (MAX_CLIENT_PER_DEVICE == 0)
#undef MAX_CLIENT_PER_DEVICE
#define MAX_CLIENT_PER_DEVICE   128
#endif

#define ZYSL_I5_MSG_LENGTH    10240 //includes all of tlv of one ieee1905 messgae for zySL_Msg_parser
#if defined (ZYSL_MULTI_STA_AIR_MONITOR)
#define ROUTINE_JOB_PERIOD       4//(sec)
#else
#define ROUTINE_JOB_PERIOD       5//(sec)
#endif
#ifndef ZY_WIFI_CHIP_ECONET
#define WBD_AP_METRIC_RESP_PERIOD 10
#endif
#define MAC_ADDR_LEN              6
#define MAX_BSS_PER_RADIO         6
#define MAX_STA_PER_BSS          64 //need to verify, used for AssocStaInfo_t & UnAssocStaInfo_t
#define MAX_LINK_NUM              4
#define EVT_BUF_LENGTH          254
#define EVT_MAX_LENGTH       (EVT_BUF_LENGTH+2)
#define BUFFER_LENGTH         32760
#define SOCKET_MAX_LENGTH    (BUFFER_LENGTH+2)
#define MAX_SSID_LENGHT      (32+1)
#define MEDIA_TYPE_LEN            2
#define VENDOR_OUI_LEN            3
#define MAX_UNASSOC_STA_NUM 16
...
#define tlv_TopologyResponseMDID_len           9  //zyxel only
...
#define MESSAGE_80211_BASE                    0x0040 // zyxel only
#define MESSAGE_STA_PROBREQ                   (MESSAGE_80211_BASE + 0x0004)
...
#define ZYSLCLI_CLIENTASSOCCTRL   0x09
/*
...
/* Evt type */
#define ZYSL_EVT_UNASSOC_STA_QRY  0x0001
...
/*========================*/
typedef struct libzySL_evt_s{
  unsigned short evt_type;
  char buffer[EVT_BUF_LENGTH];
} __attribute__((__packed__)) libzySL_evt_t;
...
typedef struct LinkMetricPair_s {
  ...
  unsigned short macthroughputcap;
...
typedef struct ApMetricsResp_s {
  unsigned short Msg_type;
  unsigned char bss_Counts;
  ApMetricsPerBss_t ApMetricsBss[MAX_BSS_PER_DEVICE];
...
typedef struct OperatingChanPerRadio_s {
...
} __attribute__((__packed__)) OperatingChanPerRadio_t;

typedef struct OperatingChanRpt_s {
  unsigned short Msg_type;
  unsigned char Radio_Counts;     //For record radio number, zyxel only
  OperatingChanPerRadio_t OperatingChanRadio[MAX_RADIO_PER_DEVICE];
...
typedef struct zySLCli_msg_s {
  ...
  unsigned char bssid[MAC_ADDR_LEN];
  unsigned char devmac[MAC_ADDR_LEN];
  int blockflag;
  int time;
...
typedef struct EvtUnAssocStaQry_s {
  unsigned short evt_type;
  unsigned char neighbor_al_mac[MAC_ADDR_LEN];
  unsigned char opClass;
  unsigned char channel;
  unsigned char sta_num;
  unsigned char sta_mac[MAX_UNASSOC_STA_NUM*MAC_ADDR_LEN];
} __attribute__((__packed__)) EvtUnAssocStaQry_t;

void zySL_Wrapper_reg(void);

#ifdef ZCFG_PLATFORM_BROADCOM
void zySL_Evt_send(void *data, int len);
int zySL_Evt_recv(libzySL_evt_t *data);
#endif

struct zySL_mesh_interface_ctrl *zySL_mesh_connection_open();
void zySL_mesh_connection_close(struct zySL_mesh_interface_ctrl *ctrl);
int zySL_mesh_ctrl_request(const char *cmd, unsigned int cmd_len);

int zySL_Socket_init(unsigned int init_port);
void zySL_Socket_deinit(int fd);
...
int tlv_ClientCapabilityReport_extract(unsigned char *pvalue, ClientCapabilityRpt_t *pCliCapRpt, unsigned short len, unsigned char *device_mac, unsigned char *ap_bssid, unsigned char *cli_mac);
int tlv_APMetrics_extract(unsigned char *pvalue, ApMetricsResp_t *pApMetricsResp, int bss_idx);
int tlv_AssociStaLinkMetric_extract(unsigned char *pvalue, ApMetricsResp_t *pApMetricsResp);
...
int tlv_OperatingChannelReport_extract(unsigned char *pvalue, OperatingChanRpt_t *pOperatingChanRpt, unsigned short len, int radio_idx);
...
int tlv_AssociStaTrafficStats_extract(unsigned char *pvalue, ApMetricsResp_t *pApMetricsResp, int sta_cnts);
#endif
